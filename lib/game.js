	...
	io.sockets.on('connection', function(user){ 
		...
		.on('kill', function ( data ) {
		 ...
		 .on('kill-ai', function ( user ) {
			...
			// kiling ai invaders shouldn't score anything :P
			addScore(10);
		...
		.on('disconnect', function(){ 
			var sid = user.id;
			var player = players[sid];
			if( typeof(player) != "undefined" ) players[sid].remove();
			// reset the defender to AI
			if( defender.name == player.name ) {
				defender = newDefender( false );
				resetGame();
			}
			//delete players[user.id].kill();
...
function enterLobby(player, socket, broadcast) {
	// exit lobby
	socket.leave('arena');
...
function resetGame() {
	...
	for( i in players ){
		...
		if(player.type=="player"){
			var sid = player.id;
			var socket = io.sockets.sockets[sid];
			enterLobby( player, socket, false);
		}
